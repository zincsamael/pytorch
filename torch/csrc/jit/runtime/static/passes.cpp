...
void EliminateTrivialEquallySplit(std::shared_ptr<torch::jit::Graph>& graph) {
  ...
  for (auto node = graph_it.next(); node != nullptr; node = graph_it.next()) {
    ...
    if(!num_splits_ivalue.has_value() || !num_splits_ivalue.value().isInt() || num_splits_ivalue.value().toInt() != 1) {
      continue;
    }
...
void FuseListUnpack(std::shared_ptr<torch::jit::Graph>& graph, const c10::FastMap<c10::Symbol, c10::Symbol>& unfused_to_fused) {
...
void FuseEquallySplitGetItemUnpack(std::shared_ptr<torch::jit::Graph>& graph) {
  ...
  for (auto node = graph_it.next(); node != nullptr; node = graph_it.next()) {
    ...
    if(!value_num_splits.has_value() || value_num_splits.value() != value_out->uses().size()) {
      continue;
    }
    ...
    for(auto index = 0; index < value_out->uses().size(); index++) {
      Node* user = value_out->uses()[index].user;
      ...
      auto getItem_index_out = constant_as<int>(user->inputs()[1]);

      // check if there is duplicate get_item use which is same output node index accessed by different get item.
      // If there is duplicate we cannot fuse
      if(!getItem_index_out.has_value() || getItem_indices.count(getItem_index_out.value()) == 1) {
        should_fuse = false;
        continue;
      }
