...
def clone_input(x, *, dtype=None):
    ...
    with torch.no_grad():
        ...
        if x.layout is torch.sparse_coo:
            return torch.sparse_coo_tensor(torch_clone(x._indices()), torch_clone(x._values()), x.shape)
        elif x.layout is torch.sparse_csr:
            return torch.sparse_csr_tensor(torch_clone(x.crow_indices()), torch_clone(x.col_indices()), torch_clone(x.values()), x.shape)
        elif x.layout is torch.sparse_csc:
            return torch.sparse_csc_tensor(torch_clone(x.ccol_indices()), torch_clone(x.row_indices()), torch_clone(x.values()), x.shape)
        elif x.layout is torch.sparse_bsr:
            return torch.sparse_bsr_tensor(torch_clone(x.crow_indices()), torch_clone(x.col_indices()), torch_clone(x.values()), x.shape)
        elif x.layout is torch.sparse_bsc:
            return torch.sparse_bsc_tensor(torch_clone(x.ccol_indices()), torch_clone(x.row_indices()), torch_clone(x.values()), x.shape)
